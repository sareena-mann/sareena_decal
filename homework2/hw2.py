...
#       An absolute path is exactly where, from start to end, a directory is located while a relative path is how to get to another directory from the directory you are currently on.
...
#       rm ./* would delete all files in current directory (not rm ./).
# 6) In your current working directory, what does calling "git add" do? What about "git commit"? What about "git push"?
#       Git add stages a change, git commit saves the changes to a local repo, and git push sends the changes to the remote repo.
...
#       It would say files are ready to commit, but they haven't been commited yet. It is telling you to commit.
